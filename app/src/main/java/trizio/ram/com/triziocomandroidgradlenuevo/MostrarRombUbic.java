                ...
                if (atributo.trim().equals("CL")) {
                    ...
                    if (atributo.trim().equals("RAZON2")) {
                        ...
                        if (atributo.trim().equals("RAZON")) {
                            ...
                            if (atributo.trim().equals("PROYECTO")) {
                                List<NameValuePair> params = new ArrayList<NameValuePair>();
                                url = "http://" + ip + "/consultarRecursoHydro.php";
                                params.add(new BasicNameValuePair("sParametro", "PROY"));
                                params.add(new BasicNameValuePair("sCodEmpresa", cod_empresa.trim()));
                                params.add(new BasicNameValuePair("sCodSucursal", cod_sucursal.trim()));
                                params.add(new BasicNameValuePair("sNit",filter.trim()));
                                String resultServer = getHttpPost(url, params);
                                System.out.println(resultServer);
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < jArray.length(); i++) {
                                        JSONObject json = jArray.getJSONObject(i);
                                        p1.add((json.getString("val_recurso").trim()));
                                        p2.add((json.getString("nom_recurso").trim()));
                                    }

                                    final String[] procesos2 = new String[p2.size()];

                                    procesos = new String[p1.size()];
                                    for (int i = 0; i < p1.size(); i++) {
                                        procesos[i] = p1.get(i);
                                    }

                                    for (int i = 0; i < p2.size(); i++) {
                                        procesos2[i] = p2.get(i);
                                    }



                                    gridview.setAdapter(new CustomGridViewAdapter(getApplicationContext(), procesos2, "grid"));// con setAdapter se llena
                                    ...
                                    gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                        ...
                                        public void onItemClick(AdapterView<?> parent, View v,
                                            ...
                                            if (procesos2[position].length() > 2) {

                                                Intent data = new Intent();
                                                data.putExtra("parametro", "proyecto");
                                                data.putExtra("valor", procesos2[position].trim());
                                                data.putExtra("descripcion", procesos[position]);
                                                //data.setDataAndType(Uri.parse(procesos[position+1].trim()),procesos[position].trim().toString());
                                                ...
                                                if (atributo.trim().equals("RAZON2")) {
                                                    ...
                                                    if (atributo.trim().equals("UBICACION")) {
                                                        ...
                                                        if (atributo.trim().equals("TECNICO")) {
                                                            ...
                                                            if (atributo.trim().equals("TIPO REQ")) {
                                                                url = "http://" + ip + "/consultarRecursoHydro.php";
                                                                params.add(new BasicNameValuePair("sParametro", "Q"));
                                                            } else {
                                                                url = "http://" + ip + "/consultarRecursoHydro.php";
                                                                params.add(new BasicNameValuePair("sParametro", "U"));
                                                            }
                                        ...
                                        try {
                                            ...
                                            gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                                ...
                                                public void onItemClick(AdapterView<?> parent, View v,
                                                    ...
                                                    if (atributo.trim().equals("TIPO REQ")) {
                                                        Intent data = new Intent();
                                                        data.putExtra("parametro", "tipreq");
                                                        data.putExtra("valor", procesos[position].trim());
                                                        //data.setDataAndType(Uri.parse(procesos[position+1].trim()),procesos[position].trim().toString());
